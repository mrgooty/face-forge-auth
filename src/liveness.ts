...
export class LivenessDetectionService {
  ...
  validateHeadMovement(
    ...
    if (livenessData.length < 2) {
      return {
        challengeType: `head_${direction}` as ChallengeType,
        requested: true,
        completed: false,
        success: false,
        confidence: 0,
  ...
  calculateOverallLivenessScore(livenessData: LivenessDetection[]): number {
    if (livenessData.length === 0) return 0;
    let score = 0;
    let factors = 0;
